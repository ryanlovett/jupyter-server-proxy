...
c.ServerProxy.servers = {
    ...
    'python-http-rewrite-response': {
        'command': ['python3', './tests/resources/httpinfo.py', '{port}'],
        'rewrite_response': lambda host, base_url, port, path, response: dict(body=response.body.replace(b"ciao", b"hello"))
    },
}

c.ServerProxy.non_service_rewrite_response = \
    lambda host, base_url, port, path, response: dict(body=response.body.replace(b"bar", b"foo"))
